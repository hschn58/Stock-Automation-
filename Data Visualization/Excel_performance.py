...
from Methods import startcol_finder, grcheck, ticks_norm
import pandas as pd
import datetime 
import math
import os
...
file_path=r"FILE_LOC"
save_loc=r"SAVE_LOC"
data_file=r"FILE_NAME"
...
while True:
    ...
    plt.bar(yr,np.array(value), zorder=3)
    ...
    ticks_norm(list(plt.xticks()[0])+year[-3:], year)
    ...
    yr=np.array(year)
    plt.grid(axis='y',linewidth=0.5, zorder=0)
    plt.bar(yr,np.array(value1), zorder=3, bottom=0)
    ...
    for i in range(lower, upper):
        ...
        if summ!=0:
            value2+=[summ]
            year+=[i]
    value2+=[sum(var[pd.DataFrame(var.columns)[0][pd.DataFrame([x[-12:] for x in var.columns])[0]==f'{upper}NetDepWD']].reset_index().iloc[:,-1])]
    year+=[upper]
    

    yr=np.array(year)
    plt.grid(axis='y',linewidth=0.5, zorder=0)
    plt.bar(yr,np.array(value2), zorder=3, bottom=0)
    ...
    plt.grid(axis='y',linewidth=0.5, zorder=0)
    plt.plot(yr,np.array(value3), linewidth=2, zorder=3)
